...
$\left(\frac{k^{1/3}}{6e^{38}\log k}\right)^k \le R_k$。那个 $e^{38}$ 是个庞大的常数，说明这条界只在 $k$ 相当大之后才非平凡——但它是**对一切 $k \ge 2$ 成立**的全称命题，不是"对充分大 $k$"。这两者在可用性上差别很大，而 Lean 逼你把区别写明白。
...
注意 $1/3$ 和 $1$ 之间还有个三倍的缺口——**这不是把问题做完了**，是把增长阶定到了 $\Theta$、常数还没合上。仓库没有假装这个缺口不存在，命题里明写着两个不同的系数。
